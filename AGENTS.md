...
- **Named methods over closures.** Business logic belongs in named methods, not anonymous functions. Closures only for trivial transforms (array_map, collection filters). Avoid PHP arrow functions (`fn() =>`) and `function () use ()` patterns — prefer `DB::beginTransaction()`/`commit()`/`rollBack()` over `DB::transaction(function () use (...) {})`.
...
- **Single-user.** One admin only + guests who don't login. Use `auth()->user()->id` to get the current user
...
- **Web responses.** `FlashMessages` trait — `$this->success($msg)` / `$this->error($msg)` -> check more functions in this trait.
